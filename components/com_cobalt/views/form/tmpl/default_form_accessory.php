            ...
            <ul class="nav nav-tabs" id="tabs-list">
                ...
                <?php if(isset($this->sorted_fields)):?>
                    <?php foreach ($this->sorted_fields as $group_id => $fields) :?>
                        ...
                        <li>
                            <a class="taberlink" href="#tab-<?php echo $group_id?>" data-toggle="tab"><?php echo HTMLFormatHelper::icon($this->field_groups[$group_id]['icon'])?> <?php echo $this->field_groups[$group_id]['name']?></a>
                        </li>
                    <?php endforeach;?>
                <?php endif;?>
...
<script>
    ...
    let elemPriceGeneral = document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_GENERAL ?>');
    elemPriceGeneral.addEventListener('blur' , calcPrices);

    // вешаем обработчик на поле ввода Спецпредложение
    document.getElementById('<?= AccessoryIds::ID_FORM_IS_SPECIAL_YES ?>').addEventListener('change' , calcPrices);
    document.getElementById('<?= AccessoryIds::ID_FORM_IS_SPECIAL_NO ?>').addEventListener('change' , calcPrices);

    // вешаем обработчик на поле ввода Оригинал
    document.getElementById('<?= AccessoryIds::ID_FORM_IS_ORIGINAL_YES ?>').addEventListener('change' , calcPrices);
    document.getElementById('<?= AccessoryIds::ID_FORM_IS_ORIGINAL_NO ?>').addEventListener('change' , calcPrices);

    // вешаем обработчик на поле ввода Под заказ
    document.getElementById('<?= AccessoryIds::ID_FORM_IS_BY_ORDER_YES ?>').addEventListener('change' , calcPrices);
    document.getElementById('<?= AccessoryIds::ID_FORM_IS_BY_ORDER_NO ?>').addEventListener('change' , calcPrices);
    ...
    let elemPriceSimple = document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_SIMPLE ?>');
    elemPriceSimple.setAttribute('readonly', 'readonly');
    let elemPriceSilver = document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_SILVER ?>');
    elemPriceSilver.setAttribute('readonly', 'readonly');
    let elemPriceGold = document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_GOLD ?>');
    elemPriceGold.setAttribute('readonly', 'readonly');
    let elemPriceDelivery = document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_DELIVERY ?>');
    elemPriceDelivery.setAttribute('readonly', 'readonly');

    // делаем наличие на складах недоступными для редактирования
    document.getElementById('<?= AccessoryIds::ID_FORM_SEDOVA ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= AccessoryIds::ID_FORM_KHIMIKOV ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= AccessoryIds::ID_FORM_ZHUKOVA ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= AccessoryIds::ID_FORM_KULTURY ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= AccessoryIds::ID_FORM_PLANERNAYA ?>').setAttribute('readonly', 'readonly');

    // если не установлена Цена спецпредложения, делаем поле ввода Спецпредложение недоступным для редактирования
    if (document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_SPECIAL ?>').value === '' || document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_SPECIAL ?>').value === '0')
    {
        document.getElementById('<?= AccessoryIds::ID_FORM_IS_SPECIAL_YES ?>').setAttribute('readonly', 'readonly');
        document.getElementById('<?= AccessoryIds::ID_FORM_IS_SPECIAL_NO ?>').setAttribute('readonly', 'readonly');
    }
    // вычисляем цены и записываем их в поля статьи
    function calcPrices() {
        let elemIsOriginal = document.getElementById('<?= AccessoryIds::ID_FORM_IS_ORIGINAL_YES ?>');
        let elemIsSpecial = document.getElementById('<?= AccessoryIds::ID_FORM_IS_SPECIAL_YES ?>');
        let elemPriceSpecial = document.getElementById('<?= AccessoryIds::ID_FORM_PRICE_SPECIAL ?>');
