...
?>
	...
	<div class="uk-grid">
		...
		<h1 class="uk-article-title"><?php echo JText::_('CNOTIFICATIONS'); ?></h1>
        </div>
	</div>
    <hr>
		...
		<div class="controls">
			<?php if($this->items) : ?>
				<div class="uk-float-right">
					<button class="uk-button uk-button-link" type="button" onclick="Cobalt.markRead('all');">
						<?php echo HTMLFormatHelper::icon('asterisk.png'); ?>
						<?php echo JText::_("CMARKALLREAD"); ?>
					</button>
					<button class="uk-button uk-button-link" type="button" data-toggle="collapse" data-target="#clear_list_block"
			...
			<?php if($this->state->get('filter.search') || (!$this->state->get('filter.search') && $this->items)):?>
				<div class="input-append">
					<input class="span2" type="text" name="filter_search" id="filter_search"
						   value="<?php echo $this->state->get('filter.search'); ?>"/>
					<button class="uk-button uk-button-link" type="submit" rel="tooltip"
					...
					<?php if($this->state->get('filter.search')) : ?>
						<button class="uk-button<?php echo($this->state->get('filter.search') ? ' btn-warning' : NULL); ?>"
								type="button"
                    ...
                    <td nowrap>
                        <div class="uk-button-group">
                            <?php if($item->state_new): ?>
                                <a class="uk-button uk-button-link" id="btn-mark-<?php echo $item->id ?>"
                                   onclick="Cobalt.markRead(<?php echo $item->id; ?>);"
                                   data-uk-tooltip title="<?php echo JText::_('CMARKASREAD') ?>">
                                   <?php echo HTMLFormatHelper::icon('asterisk.png'); ?>
                                </a>
                            <?php endif; ?>
