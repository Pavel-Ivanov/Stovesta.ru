...
$file_path = JPATH_ROOT . "/logs/b0_set_empty_description_log.txt";
$file_handle = fopen($file_path, 'ab+');
if (!$file_handle) {
    $app->enqueueMessage('B0 clear meta - Ошибка открытия файла b0_set_empty_description_log.txt');
    return;
}
$res = fwrite($file_handle, '===== ' . date_format(new DateTime(), 'Y-m-d H:i:s') . ' =====' . "\n");

if (!$res) {
    $app->enqueueMessage('B0 add value - Ошибка записи в файл b0_set_empty_description_log.txt');
    fclose($file_handle);
    return;
}
...
foreach($items as $item) {
    ...
    if ($item->title === $item->meta_descr) {
        if ($isUpdate === '1') {
            $query="UPDATE #__js_res_record SET meta_descr='' WHERE id=$item->id";
            $db->setQuery($query);
            $result = $db->execute();
            if (!$db->execute()) {
