...
//		}
		...
		switch($type) {
			case 2:
				...
				$html .= $this->_render_autocomplete($multi, [], $default,
...
/*		if(!in_array($this->params->get('params.strict_to_user'), JFactory::getUser()->getAuthorisedViewLevels()))
		...
		<div id="parent_list<?= $this->id; ?>"></div>
		<a data-toggle="modal" role="button" class="btn btn-small btn-warning"
		   href="#modal<?= $this->id; ?>"><?= JText::_($this->params->get('params.control_label')) ?></a>

		<div style="width:770px;" class="modal hide fade" id="modal<?= $this->id; ?>" tabindex="-1"
			 role="dialog">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				<h3 id="myModalLabel"><?= $app->input->get('view') == 'records' ? JText::_('Select Children') : JText::_('FS_ATTACHEXIST'); ?></h3>
			</div>
		...
		<script>
			(function ($) {
				window.modal<?= $this->id; ?> = $('#modal<?= $this->id; ?>');
				window.elementslist<?= $this->id; ?> = $('#parent_list<?= $this->id; ?>');
				window.multi<?= $this->id; ?> = <?= $multi ? 'true' : 'false';?>;
				window.limit<?= $this->id; ?> = <?= $this->params->get('params.multi_limit', 0);?>;
				window.name<?= $this->id; ?> = '<?= $name; ?>';

				$('#modal<?= $this->id;?>').on('show', function () {
					var ids = [];
					$.each(elementslist<?= $this->id; ?>.children('div.alert'), function (k, v) {
						ids.push($(v).attr('rel'));
					});
					...
					var iframe = $(document.createElement("iframe")).attr({
						...
						width: "100%",
						height: "510px",
						src: '<?= $doTask;?>&excludes=' + ids.join(',')
					});
					$(".modal-body").html(iframe);
				});

				window.list<?= $this->id; ?> = function (id, title) {
					<?php if(!$multi):?>
                        elementslist<?= $this->id; ?>.html('');
					<?php else: ?>
					lis = elementslist<?= $this->id; ?>.children('div.alert');
					if (lis.length >= limit<?= $this->id; ?>) {
						alert('<?= JText::sprintf('CSELECTLIMIT', $this->params->get('params.multi_limit'));?>');
						return false;
					}
					error = 0;
					...
					var el = $(document.createElement('div')).attr({
						...
						rel: id
					}).html('<a class="close" data-dismiss="alert" href="#">x</a><span>' + title + '</span><input type="hidden" name="<?= $name ?>" value="' + id + '">');
					elementslist<?= $this->id; ?>.append(el);
					return true;
				}

				<?php foreach ($default as $item): ?>
				list<?= $this->id; ?>(<?= $item->id; ?>, '<?= htmlspecialchars($item->title, ENT_COMPAT, 'UTF-8')?>');
				<?php endforeach;?>

				window.updatelist<?= $this->id; ?> = function (list) {
					var elementslist<?= $this->id; ?> = $('#parent_list<?= $this->id; ?>');
					elementslist<?= $this->id; ?>.empty();
					$.each(list, function () {
						elementslist<?= $this->id; ?>.append(this);
					});
				}
			}(jQuery));
		</script>
	...
	public function validate($value, $record, $type, $section)
	{
		if($this->params->get('params.multi_limit')) {
			...
			if(count((array)$value) > $this->params->get('params.multi_limit')) {
	...
	{
		$app = JFactory::getApplication();
		return $app->getUserState('skipers.all', []);
	}
