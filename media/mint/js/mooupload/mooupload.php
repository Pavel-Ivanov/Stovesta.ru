...
class Mooupload
{
	...
	public function HTML5_upload()
	{
		...
		{
			$response['error'] = JText::sprintf($headers['X-File-Id'].' / HTML5 File %s have unallowed extension %s', $response['name'], $ext);
	...
	public function _convert_size($val)
	{
		...
		$first = substr($val, 0, strlen($val) - 1);

		switch($last)
		{
			case 'g' :
				$val = $first * 1024000000;
				break;
			case 'm' :
				$val = $first * 1024000;
				break;
