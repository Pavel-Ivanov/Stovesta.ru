...
class JFormFieldCImage extends CFormField
{
	...
	{
        $defaultImg = str_replace('\\', '/', $this->params->get('params.default_img', ''));
        if (!$this->value) {
            $this->value = [];
        }
		if(empty($this->value['image']) && $defaultImg) {
			$this->value['image'] = $defaultImg;
		}
		if(!empty($this->value['image'])) {
	...
	public function deleteImage()
	{
		...
		if($out['success'] > 0 && $record_table->id && $files_table->saved) {
			$fields = json_decode($record_table->fields, TRUE, 512, JSON_THROW_ON_ERROR);

			if(isset($fields[$this->id])) {
				unset($fields[$this->id]);
				$record_table->fields = json_encode($fields, JSON_THROW_ON_ERROR);
				$record_table->store();
			}
		}
