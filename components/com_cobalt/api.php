...
class CobaltApi
{
	...
	public static function renderField($record, $field_id, $view, $default = null, $bykey = false)
	{
		...
		if(!method_exists($fieldclass, $func)) {
			throw new RuntimeException(JText::_('AJAX_METHODNOTFOUND'), 500);
		}
	...
	public static function updateRecord($record_id, $data, $fields = [], $categories = [], $tags = [])
	{
		...
		if(!$record->id) {
			throw new RuntimeException("Cobalt API: update Record: Record not found", 500);
		}
	...
	{
		try {
			/* @var CobaltTableRecord_values $tableRecordValues */
			$tableRecordValues = JTable::getInstance('Record_values', 'CobaltTable');
			$type = ItemsStore::getType($record->type_id);
			$db = JFactory::getDbo();
			...
			if($fields) {
                ...
                $tableRecordValues->clean($record->id, $fieldsIds);

                /** @var CobaltModelFields $modelFields */
                $modelFields = JModelLegacy::getInstance('Fields', 'CobaltModel');
				$form_fields  = $modelFields->getFormFields($record->type_id, $record->id, false, $fields);

				$validData['id'] = $record->id;

				foreach($form_fields as $key => $field) {
					if(!in_array((int)$field->id, $fieldsIds, true)) {
						continue;
					}
                    ...
                    foreach($values as $key1 => $value) {
						$tableRecordValues->store_value($value, $key1, $record, $field);
						$tableRecordValues->reset();
						$tableRecordValues->id = null;
					}
				}
			...
			if($categories) {
                /** @var CobaltTableCobCategory $tableCategory */
                $tableCategory = JTable::getInstance('CobCategory', 'CobaltTable');
                /** @var CobaltTableRecord_category $tableRecordCategory */
                $tableRecordCategory = JTable::getInstance('Record_category', 'CobaltTable');

				$categoriesIds = [];
				foreach($categories as $key) {
					$tableCategory->load($key);
					...
					$tableRecordCategory->load($array);

					if(!$tableRecordCategory->id) {
						$array['published'] = $tableCategory->published;
						$array['access']    = $tableCategory->access;
						$array['id']        = NULL;
						$tableRecordCategory->save($array);
					}
					else {
						$tableRecordCategory->published = $tableCategory->published;
						$tableRecordCategory->access    = $tableCategory->access;
						$tableRecordCategory->store();
					}

					$categoriesIds[] = $key;
					$cat_save[$key] = $tableCategory->title;

					$tableRecordCategory->reset();
					$tableRecordCategory->id = NULL;
				}

				if($categoriesIds) {
					$sql = 'DELETE FROM #__js_res_record_category WHERE record_id = ' . $record->id . ' AND catid NOT IN (' . implode(',', $categoriesIds) . ')';
					$db->setQuery($sql);
					$db->execute();
				}

				$record->categories = json_encode($cat_save, JSON_THROW_ON_ERROR);
			}
