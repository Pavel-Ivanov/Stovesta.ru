...
$file_path = JPATH_ROOT . "/logs/b0_clear_meta_log.txt";
$file_handle = fopen($file_path, 'ab+');
if (!$file_handle) {
	$app->enqueueMessage('B0 clear meta - Ошибка открытия файла b0_clear_meta_log.txt');
	return;
}
...
foreach($items as $item) {
    ...
    if ($isUpdate === '1') {
        $query="UPDATE #__js_res_record SET meta_descr='', meta_key='' WHERE id={$item->id}";
        $db->setQuery($query);
        $result = $db->execute();
        if (!$db->execute()) {
