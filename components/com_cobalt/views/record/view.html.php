...
class CobaltViewRecord extends JViewLegacy
{
	public function display($tpl = null)
	{
		...
		// Получаем запись из таблицы __js_res_record
		$item = $this->get('Item');
		...
		if(!$this->checkSectionAccess($section,$user)) {
			throw new Exception(JText::_($section->params->get('general.status_msg')), 404);
		}
		...
		if($type->published == 0) {
			throw new Exception(JText::_('CMSG_TYPEUNPUB'), 404);
		}
		...
		// Добавляем дополнительную информацию в $item
	...
	private function checkSectionAccess($section, $user)
	{
		if($section->published === '0') {
			throw new Exception(JText::_('CERR_SECTIONUNPUB'), 404);
		}
