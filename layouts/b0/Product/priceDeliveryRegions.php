...
<?php else:?>
    <?php $delta = (int) $displayData->priceGeneral['value'] - (int) $displayData->priceDeliveryRegions['value'];?>
    <?php if ($delta > 0): ?>
        <p class="b0-price b0-price-first uk-text-danger">
            <?= 'Цена при доставке : ' . $displayData->priceDeliveryRegions['result'] ?>
        </p>
        <p class="b0-price b0-price-second">
            <?= 'Цена : ' . $displayData->priceGeneral['result'] ?>
        </p>
    ...
    <?php else : ?>
        <p class="b0-price b0-price-first">
            <?= 'Цена: ' . $displayData->priceDeliveryRegions['result'] ?>
        </p>
    <?php endif; ?>
<?php endif;?>
