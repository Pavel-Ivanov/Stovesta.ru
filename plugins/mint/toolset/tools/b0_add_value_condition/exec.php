...
//Поле, в котором ищем контрольные значения $value_to_search
if (!$params['field_id_to_search']) {
	$app->enqueueMessage('Не выбрано поле, в котором искать');
	return;
}
$field_id_to_search =$params['field_id_to_search'];
...
$file_path = JPATH_ROOT . "/logs/b0_add_value_condition_log.txt";
$file_handle = fopen($file_path, 'ab+');
if ($file_handle == false) {
	$app->enqueueMessage('B0 add value by condition - Ошибка открытия файла add_value_condition_log.txt');
	return;
}
...
$res = fwrite($file_handle, 'Добавляем: '. $value_to_add .' если установлен: '.$value_to_search . "\n");

// Получаем записи из БД
$db  = JFactory::getDbo();
$query = "SELECT * FROM #__js_res_record WHERE section_id={$section_id} AND id IN(SELECT record_id FROM #__js_res_record_values WHERE field_id={$field_id_to_search}";
if ($is_exact_search) {
...
/*	if(empty($fields[$field_id_to_add])) {
	...
	if (in_array($value_to_add, $fields[$field_id_to_add], true)) {
		$str .= ' / уже установлено, пропускаем';
		fwrite($file_handle, $str . "\n");
		continue;
	}

	$fields[$field_id_to_add][] = $value_to_add;
