...
<script>
    'use strict';
    // определяем константы полей
    const keyElemPriceGeneral = 'field_37';
    const keyElemPriceSimple = 'field_67';
    const keyElemPriceSilver = 'field_68';
    const keyElemPriceGold = 'field_69';
    const keyElemPriceFirstVisit = 'field_139';
    const keyElemPriceSpecial = 'field_38';
    const keyBoolYesIsSpecial = 'boolyes66';
    const keyBoolNoIsSpecial = 'boolno66';

    // определяем элементы интерфейса
    let elemPriceGeneral = document.getElementById(keyElemPriceGeneral);
    let elemPriceSimple = document.getElementById(keyElemPriceSimple);
    let elemPriceSilver = document.getElementById(keyElemPriceSilver);
    let elemPriceGold = document.getElementById(keyElemPriceGold);
    let elemPriceFirstVisit = document.getElementById(keyElemPriceFirstVisit);
    let elemPriceSpecial = document.getElementById(keyElemPriceSpecial);
    let elemBoolYesIsSpecial = document.getElementById(keyBoolYesIsSpecial);
    let elemBoolNoIsSpecial = document.getElementById(keyBoolNoIsSpecial);

    // вешаем обработчик на поле ввода Цена
    elemPriceGeneral.addEventListener('blur' , calcPrices);

    // вешаем обработчик на поле Цена Спецпредложения
    elemPriceSpecial.addEventListener('blur' , setSpecial);
    ...
    elemPriceFirstVisit.setAttribute('readonly', 'readonly');

    // делаем поле ввода Спецпредложение недоступным для редактирования, если Цена спецпредложения не установлена или равна 0
    if (elemPriceSpecial.value === '' || elemPriceSpecial.value === '0') {
        elemBoolYesIsSpecial.setAttribute('readonly', 'readonly');
        elemBoolNoIsSpecial.setAttribute('readonly', 'readonly');
    }
    else {
        // вешаем обработчик на поле ввода Спецпредложение
        elemBoolYesIsSpecial.addEventListener('change' , calcPrices);
        elemBoolNoIsSpecial.addEventListener('change' , calcPrices);
    }

    // вычисляем цены и записываем их в поля статьи
    function calcPrices() {
        if (elemBoolYesIsSpecial.checked === true) {
            ...
            elemPriceFirstVisit.value = elemPriceSpecial.value;
        }
        else {
            elemPriceSimple.value = (elemPriceGeneral.value*0.97).toFixed();
            elemPriceSilver.value = (elemPriceGeneral.value*0.95).toFixed();
            elemPriceGold.value = (elemPriceGeneral.value*0.93).toFixed();
            elemPriceFirstVisit.value = (elemPriceGeneral.value*0.75).toFixed();
        }
    }

    function setSpecial() {
        if (elemPriceSpecial.value === '0') {
            elemBoolYesIsSpecial.setAttribute('readonly', 'readonly');
            elemBoolNoIsSpecial.setAttribute('readonly', 'readonly');

            elemBoolYesIsSpecial.removeEventListener('change' , calcPrices);
            elemBoolNoIsSpecial.removeEventListener('change' , calcPrices);
        }
        else {
            elemBoolYesIsSpecial.removeAttribute('readonly');
            elemBoolNoIsSpecial.removeAttribute('readonly');
