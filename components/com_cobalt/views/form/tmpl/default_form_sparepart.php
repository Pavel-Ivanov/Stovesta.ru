<?php
// tmpl_params_form_grouping_type =1 - Вкладки
defined('_JEXEC') or die();
JImport('b0.Sparepart.SparepartIds');
JImport('b0.Company.PriceConfig');
...
<div class="form-horizontal uk-margin">
    <?php if(in_array($params->get('tmpl_params.form_grouping_type', 0), array(1, 4))):?>
        <div class="tabbable<?php if($params->get('tmpl_params.form_grouping_type', 0) == 4) echo ' tabs-left'; ?>">
            <ul class="nav nav-tabs" id="tabs-list">
                <li>
                    <a href="#tab-main" data-toggle="tab"><?php echo JText::_($params->get('tmpl_params.tab_main', 'Main'));?></a>
                </li>
                <?php if(isset($this->sorted_fields)):?>
                    <?php foreach ($this->sorted_fields as $group_id => $fields) :?>
                        <?php if($group_id == 0) continue;?>
                        <li>
                            <a class="taberlink" href="#tab-<?php echo $group_id?>" data-toggle="tab">
                                <?php echo HTMLFormatHelper::icon($this->field_groups[$group_id]['icon'])?> <?php echo $this->field_groups[$group_id]['name']?>
                            </a>
                        </li>
                    <?php endforeach;?>
                <?php endif;?>
                <?php if(count($this->meta)):?>
                    <li>
                        <a href="#tab-meta" data-toggle="tab"><?php echo JText::_('Метаданные');?></a>
                    </li>
                <?php endif;?>
                <?php if(count($this->core_admin_fields)):?>
                    <li>
                        <a href="#tab-special" data-toggle="tab"><?php echo JText::_('Специальные');?></a>
                    </li>
                <?php endif;?>
                <?php if(count($this->core_fields)):?>
                    <li>
                        <a href="#tab-core" data-toggle="tab"><?php echo JText::_('Core Fields');?></a>
                    </li>
                <?php endif;?>
            </ul>
    <?php endif;?>
	...
	<?php if(count($this->core_fields)):?>
		...
		<?php foreach($this->core_fields as $key => $field ):?>
			<div class="control-group odd<?= $k = 1 - $k ?>">
				<label id="jform_<?= $field?>-lbl" class="control-label" for="jform_<?= $field?>" >
					<?php if($params->get('tmpl_core.form_'.$field.'_icon', 1)):?>
						<?= HTMLFormatHelper::icon('core-'.$field.'.png') ?>
					<?php endif;?>
					<?= strip_tags($this->form->getLabel($field)) ?>
				</label>
				<div class="controls">
					<?= $this->form->getInput($field) ?>
				</div>
			</div>
		<?php endforeach;?>
		<?php group_end($this);?>
...
<script>
    // вешаем обработчик на поле ввода Цена
    let elemPriceGeneral = document.getElementById('<?= SparepartIds::ID_FORM_PRICE_GENERAL ?>');
    elemPriceGeneral.addEventListener('blur' , calcPrices);

    // вешаем обработчик на поле ввода Спецпредложение
    document.getElementById('<?= SparepartIds::ID_FORM_IS_SPECIAL_YES ?>').addEventListener('change' , calcPrices);
    document.getElementById('<?= SparepartIds::ID_FORM_IS_SPECIAL_NO ?>').addEventListener('change' , calcPrices);

    // вешаем обработчик на поле ввода Оригинал
    document.getElementById('<?= SparepartIds::ID_FORM_IS_ORIGINAL_YES ?>').addEventListener('change' , calcPrices);
    document.getElementById('<?= SparepartIds::ID_FORM_IS_ORIGINAL_NO ?>').addEventListener('change' , calcPrices);

    // вешаем обработчик на поле ввода Под заказ
    document.getElementById('<?= SparepartIds::ID_FORM_IS_BY_ORDER_YES ?>').addEventListener('change' , calcPrices);
    document.getElementById('<?= SparepartIds::ID_FORM_IS_BY_ORDER_NO ?>').addEventListener('change' , calcPrices);

    // вешаем обработчик на длину Заголовка
    let elemTitle = document.getElementById("jform_title");
    document.getElementById("title-length").innerText = '(' + elemTitle.value.length + ')';
    elemTitle.onkeyup = function() {
        document.getElementById("title-length").innerText = '(' + elemTitle.value.length + ')';
    };

    // делаем расчетные цены недоступными для редактирования
    let elemPriceSimple = document.getElementById('<?= SparepartIds::ID_FORM_PRICE_SIMPLE ?>');
    elemPriceSimple.setAttribute('readonly', 'readonly');
    let elemPriceSilver = document.getElementById('<?= SparepartIds::ID_FORM_PRICE_SILVER ?>');
    elemPriceSilver.setAttribute('readonly', 'readonly');
    let elemPriceGold = document.getElementById('<?= SparepartIds::ID_FORM_PRICE_GOLD ?>');
    elemPriceGold.setAttribute('readonly', 'readonly');
    let elemPriceDelivery = document.getElementById('<?= SparepartIds::ID_FORM_PRICE_DELIVERY ?>');
    elemPriceDelivery.setAttribute('readonly', 'readonly');

    // делаем наличие на складах недоступными для редактирования
    document.getElementById('<?= SparepartIds::ID_FORM_SEDOVA ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= SparepartIds::ID_FORM_KHIMIKOV ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= SparepartIds::ID_FORM_ZHUKOVA ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= SparepartIds::ID_FORM_KULTURY ?>').setAttribute('readonly', 'readonly');
    document.getElementById('<?= SparepartIds::ID_FORM_PLANERNAYA ?>').setAttribute('readonly', 'readonly');

    // если не установлена Цена спецпредложения, делаем поле ввода Спецпредложение недоступным для редактирования
    if (document.getElementById('<?= SparepartIds::ID_FORM_PRICE_SPECIAL ?>').value === '' || document.getElementById('<?= SparepartIds::ID_FORM_PRICE_SPECIAL ?>').value === '0')
    {
        document.getElementById('<?= SparepartIds::ID_FORM_IS_SPECIAL_YES ?>').setAttribute('readonly', 'readonly');
        document.getElementById('<?= SparepartIds::ID_FORM_IS_SPECIAL_NO ?>').setAttribute('readonly', 'readonly');
    }
    // вычисляем цены и записываем их в поля статьи
    function calcPrices() {
        let elemIsOriginal = document.getElementById('<?= SparepartIds::ID_FORM_IS_ORIGINAL_YES ?>');
        let elemIsSpecial = document.getElementById('<?= SparepartIds::ID_FORM_IS_SPECIAL_YES ?>');
        let elemPriceSpecial = document.getElementById('<?= SparepartIds::ID_FORM_PRICE_SPECIAL ?>');

        if (elemIsSpecial.checked === true) {
            elemPriceSimple.value = elemPriceSpecial.value;
            elemPriceSilver.value = elemPriceSpecial.value;
            elemPriceGold.value = elemPriceSpecial.value;
            elemPriceDelivery.value = elemPriceSpecial.value;
            return;
        }

        if (elemIsOriginal.checked === true) {
            elemPriceSimple.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_ORIGINAL_SIMPLE ?>).toFixed();
            elemPriceSilver.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_ORIGINAL_SILVER ?>).toFixed();
            elemPriceGold.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_ORIGINAL_GOLD ?>).toFixed();
            elemPriceDelivery.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_ORIGINAL_DELIVERY ?>).toFixed();
        }
        else {
            elemPriceSimple.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_NON_ORIGINAL_SIMPLE ?>).toFixed();
            elemPriceSilver.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_NON_ORIGINAL_SILVER ?>).toFixed();
            elemPriceGold.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_NON_ORIGINAL_GOLD ?>).toFixed();
            elemPriceDelivery.value = (elemPriceGeneral.value*<?= PriceConfig::PRICE_DISCOUNT_NON_ORIGINAL_DELIVERY ?>).toFixed();
        }
    }
</script>
