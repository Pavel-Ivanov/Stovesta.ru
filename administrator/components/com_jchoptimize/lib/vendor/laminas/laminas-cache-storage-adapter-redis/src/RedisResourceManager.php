...
final class RedisResourceManager
{
     ...
     *
     * @param string                           $id
     * @param array|RedisResource|\Traversable $resource
     *
     ...
     */
    private function normalizeServer(&$server)
    {
        $host = null;
        $port = null;
        $timeout = 0;
     ...
     * @return null|string
     */
    private function extractPassword($resource, $serverUri)
    {
        if (!empty($resource['password'])) {
            return $resource['password'];
        }
     ...
     * @throws Exception\RuntimeException
     */
    private function connect(array &$resource)
    {
     ...
     * @param string $persistentId
     */
    private function normalizePersistentId(&$persistentId)
    {
     ...
     * @throws Exception\InvalidArgumentException
     */
    private function normalizeLibOptions(&$libOptions)
    {
     ...
     * @throws Exception\InvalidArgumentException
     */
    private function normalizeLibOptionKey(&$key)
    {
        // convert option name into it's constant value
        if (\is_string($key)) {
