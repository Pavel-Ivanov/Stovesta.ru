...
class YmlOffer
{
	...
	public function __construct(object $item, $categories, string $mode)
	{
		$this->id = $item->id;
...
//		$this->availability = $this->getAvailability();
	...
	private function getCategoryId($item, array $categories)
	{
        try {
...
/*				if (isset($this->fields[SparepartIds::ID_YM_IMAGE]['image'])) {
				...
				if (isset($this->fields[SparepartIds::ID_IMAGE]['image'])) {
					...
					return YmlConfig::YML_URL_NO_IMAGE;
				}
			case AccessoryIds::ID_SECTION:
...
/*	private function getAvailability(): int
	...
	public function renderOffer($mode) :string
	{
...
/*        if ($mode === YmlConfig::YML_MODE_MARKET || $mode === YmlConfig::YML_MODE_OFFERS_ONLY) {
...
//		$content .= $this->renderParams();
	...
	private  function renderUrl():string
	{
		switch ($this->sectionId) {
			case SparepartIds::ID_SECTION:
				return '<url>' . YmlConfig::YML_URL_SPAREPARTS . $this->id . '-' . $this->alias . '</url>' . "\n";
			case AccessoryIds::ID_SECTION:
				return '<url>' . YmlConfig::YML_URL_ACCESSORIES . $this->id . '-' . $this->alias . '</url>' . "\n";
			default:
				return '';
		}
	}
...
//		$price = $this->isSpecial && $this->priceDiscount >= 5 ? $this->priceSpecial : $this->priceGeneral;
	...
	{
		$price = ($this->isSpecial) ? $this->priceSpecial : $this->priceGeneral;
		if ($price >= YmlConfig::YML_LIMIT_DELIVERY_CITY) {
			return '';
		}
		return '<sales_notes>' . YmlConfig::YML_SALES_NOTES_DELIVERY . '</sales_notes>' . "\n";
	}
	...
	{
		return '<picture>'.YmlConfig::YML_URL.'/'.$this->imageUrl.'</picture>' . "\n";
	}
...
//		if ($price >= YmlConfig::YML_LIMIT_DELIVERY_CITY) {
		if ($price >= YmlConfig::YML_LIMIT_DELIVERY_SATELLITES) {
			return '';
		}
		$str = '<delivery-options>' . "\n";
		$str .= '<option cost="'.YmlConfig::YML_PRICE_DELIVERY_SATELLITES.'" days="'.YmlConfig::YML_DELIVERY_OPTIONS[0]['days'].'"/>' . "\n";
		$str .= '</delivery-options>' . "\n";
		return $str;
	}*/
